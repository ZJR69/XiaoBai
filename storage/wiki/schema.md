...
└─ wiki/                # 小白维护的知识 wiki
   ├─ schema.md        # 本手册
   ├─ core.md          # 底座记忆：用户是谁、组织性质、战略定位、关键人物、长期方向（对话全文注入，保持精炼）
...
- 按 `## 小节标题` 组织（如：## 组织与战略 / ## 关键人物 / ## 长期方向）
- 同名小节覆盖写入，新小节追加——更新即覆盖，不留历史版本（历史在 log.md）
- **保持精炼**：每次对话全文注入，文件越长注意力越稀释；判据：删掉一条会不会导致具体错误，不会就删
- 修改必须经用户裁决（提案 kind=core）
